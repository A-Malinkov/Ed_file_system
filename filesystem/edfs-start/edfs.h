...
/* Size limits (for mkfs). */
#define EDFS_MAX_BLOCKS (1 << ((sizeof(edfs_block_t) * 8) - 1))

#define EDFS_MAX_BLOCK_SIZE (1 << 13)
#define EDFS_MIN_BLOCK_SIZE (1 << 9)

/* Block 0 is reserved to mean "invalid". */
...
/* The super block is always stored at offset 512 from the start of the disk.
 */
...
typedef struct
{
  ...
  uint16_t block_size;  /* technically supports blocks up to 32 KiB, but we cap
                         * at 8 KiB, EDFS_MAX_BLOCK_SIZE.
                         */
...
typedef enum
{
  ...
  EDFS_INODE_TYPE_INDIRECT = 1 << 7 /* Flag to indicate block pointers are
                                     * indirect blocks. */
} edfs_inode_type_t;
...
/* Inode 0 is reserved to mean "invalid". */
#define EDFS_INODE_INVALID 0

/* Padded to be 16 bytes in size, with 7 reserved bytes available for future
 * expansion.
 */
typedef struct
{
...
/* Filename includes a null-terminator, so the effective size of a filename is
 * 59 bytes.
 */
...
/* Returns the offset of the super block.
 */
static inline uint32_t
edfs_get_super_block_offset(void)
{
  /* Super block is always located at offset 512, after the boot block */
  return 512;
}

/* Returns the size in bytes of the file system.
 */
static inline uint32_t
...
/* Returns the amount of directory entries that fit in a single block.
 */
static inline int
...
/* Returns the amount of block numbers that fit in a single block.
 */
static inline int
...
/* Returns the offset in bytes of a given block number @block.
 */
static inline uint32_t
...
/* Returns whether the directory entry @entry is unused.
 */
static inline bool
...
/* Returns whether the inode @inode is a directory.
 */
static inline bool
edfs_inode_is_directory(const edfs_disk_inode_t *inode)
{
  return inode->type == EDFS_INODE_TYPE_DIRECTORY;
}

/* Returns whether the inode @inode uses indirect blocks.
 */
static inline bool
edfs_inode_has_indirect(const edfs_disk_inode_t *inode)
{
